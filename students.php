...
if(!isset($_SESSION['username'])){
    if(isset($_POST['submit'])){//用户提交登录表单时执行如下代码
        ...
        if(!empty($user_username)&&!empty($user_password)){
            ...
            $query = "SELECT njs_studentUserNo_05 FROM nongjs_studentuser_05 WHERE njs_studentUserNo_05 = '$user_username' AND "."njs_studentUserPassword_05 = '$user_password'";
            //用用户名和密码进行查询
            $data = mysqli_query($dbc,$query);
            //若查到的记录正好为一条，则设置SESSION，同时进行页面重定向
            if(mysqli_num_rows($data)==1){
                $row = mysqli_fetch_array($data);
                $_SESSION['username']=$row['njs_studentUserNo_05'];
