...
function get_id(){
    ...
    if (empty($_GET['id'])) {
        ...
        echo "<meta http-equiv='Refresh' content='0;URL=../admin/major_info.php'>";
    }
    $id = $_GET['id'];
    ...
    $sql="select * from `$db_table` where njs_majorNo_05 = '$id' limit 1";
...
function update_major(){
    ...
    $sql = "UPDATE `$db_table` SET njs_majorNo_05='$id',njs_majorName_05='$name' where njs_majorNo_05='$id'";
    $query = mysqli_query($link,$sql);
    if (!$query) {
...
<div class="container edit">
    <h4 class="alert alert-primary text-center">修改专业信息</h4>
    <form method="post" action="major_update.php">
        <div class="form-group row">
            <label  class="col-sm-2 control-label">专业编号</label>
            <div class="col-sm-10">
                <input class="form-control" readonly="readonly" type="text" id="major_id" name="major_id" value="<?php echo $user['njs_majorNo_05']; ?>"/>
            </div>
        </div>
        <div class="form-group row">
            <label  class="col-sm-2 control-label">专业名称</label>
            <div class="col-sm-10">
                <input class="form-control" type="text" id="major_name" name="major_name" value="<?php echo $user['njs_majorName_05']; ?>"/>
            </div>
        </div>
